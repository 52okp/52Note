...
- `qianduan/app/stage/protyle/js/lute/lute.min.js` (built from upstream `88250/lute`)
- `qianduan/app/stage/build/**`, `qianduan/app/src/types/dist/**`
- `qianduan/app/changelogs/**` (generated by separate tooling)
- `qianduan/app/kernel/SiYuan-Kernel*`, `*.syso`, `qianduan/kernel/kernel.aar`
- `qianduan/app/pandoc/*`

### Verification and prohibited operations

1. **Frontend verification:** Do not use `npx webpack` or `pnpm dev` to verify changes; after changes, run `pnpm run lint` with `qianduan/app/` as the working directory to check code style
...
3. **Icons:** Do not hand-write SVG; use existing icons from `qianduan/app/appearance/icons/litheness/icon.js` when possible
   ...
   - When adding an icon to `qianduan/app/appearance/icons/litheness/icon.js`, add its preview entry to `qianduan/app/appearance/icons/index.html` in the same change and keep the order aligned
...
9. **Configurable entries:**
   ...
   - When adding, removing, renaming, or moving a configurable desktop menu item or dock entry, or changing its `data-id` / `data-type`, update `qianduan/app/src/config/entryVisibility/catalog.ts` in the same change, including its type, hierarchy, label, Simple profile default, and default position, and update the related tests
...
|---|---|---|
| Go | see `go` directive | `qianduan/kernel/go.mod` |
| Node (+ pnpm) | see CI matrix | `.github/workflows/cd.yml`, `qianduan/app/package.json` (`packageManager` field) |

---

## 5. Repository layout

**Architecture:** The client consists of a local Go kernel (`qianduan/kernel/`) and TypeScript frontend (`qianduan/app/`), plus a separate `export` bundle (global `Protyle`, entry `src/protyle/method.ts`) for rendering rich content in exported HTML / PDF preview. The private cloud backend is maintained in a separate repository and is outside this repository's scope. Read client versions from `qianduan/kernel/go.mod`, `qianduan/app/package.json`, `qianduan/kernel/util/working.go`.

Top level (repo root):

| Path | Contents |
|---|---|
| `qianduan/kernel/` | Client-local Go kernel — local server, data engine, API, and domain logic |
| `qianduan/app/` | TypeScript frontend (Electron/web), built by webpack into `qianduan/app/stage/build/` |
| `qianduan/scripts/` | Client release and packaging scripts |
| `qianduan/app/appearance/` | Themes, icons, **i18n** (`appearance/langs/*.json`) |
| `qianduan/app/stage/` | Build output served by the kernel |
| `qianduan/app/changelogs/` | Per-version changelog markdown |
| `.github/` | `CONTRIBUTING.md` (+zh-CN), `SECURITY.md`, `CODE_OF_CONDUCT.md`, `PULL_REQUEST_TEMPLATE.md`, issue templates, `workflows/` |

### Major `qianduan/kernel/` packages (under `qianduan/kernel/`)
...
### Frontend (`qianduan/app/src/`) highlights
...
| `go-sqlite3` / `pdfcpu` | Maintainer's forks, pulled in via permanent `replace` in `qianduan/kernel/go.mod` (keep those) |
...
All Go libraries above are dependencies in `qianduan/kernel/go.mod`. GitHub org: `siyuan-note/*` for the `siyuan-` apps and most libs; `88250/*` for lute, gulu, and the forks (go-sqlite3 / pdfcpu).

### Cross-repo notes

- **Editing any Go dependency (Lute / dejavu / gulu / eventbus / riff / filelock / httpclient / logging / go-sqlite3 / pdfcpu / epub / …):** these are imported by the kernel as Go modules (`qianduan/kernel/go.mod`). To test a local change, add a temporary `replace` in `qianduan/kernel/go.mod` pointing at your local checkout — but **never commit that `replace`**; it breaks builds for everyone else.
- **Rebuilding `lute.min.js`:** it's the JS build of the Go `lute` project — generated upstream and checked into `qianduan/app/stage/protyle/js/lute/`. Don't edit it here; change `lute`, rebuild, and copy the artifact in.
- **Type declarations:** when changing files under `qianduan/app/src/types/` or other TypeScript declarations and constants exposed to plugins, synchronize the corresponding declarations and constants in the `petal` repository in the same task.
