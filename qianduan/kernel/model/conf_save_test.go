...
func TestNewAppConfPreservesCloseButtonBehaviorDefault(t *testing.T) {
	app := NewAppConf()
	if app.Appearance.CloseButtonBehavior != 1 {
		t.Fatalf("unexpected close button behavior default: %d", app.Appearance.CloseButtonBehavior)
	}

	if err := gulu.JSON.UnmarshalJSON([]byte(`{"appearance":{"mode":1}}`), app); err != nil {
		t.Fatal(err)
	}
	if app.Appearance.CloseButtonBehavior != 1 {
		t.Fatalf("legacy config should preserve the close button behavior default: %d", app.Appearance.CloseButtonBehavior)
	}

	if err := gulu.JSON.UnmarshalJSON([]byte(`{"appearance":{"closeButtonBehavior":0}}`), app); err != nil {
		t.Fatal(err)
	}
	if app.Appearance.CloseButtonBehavior != 0 {
		t.Fatalf("explicit close button behavior should be preserved: %d", app.Appearance.CloseButtonBehavior)
	}
}
